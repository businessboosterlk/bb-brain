...
/* ── THE PILLARS (2026-09-07, Thulaib: "when we want to open a new business we have
   ...
   every client, graded by how many CLIENTS proved it. This reads the same blocks the
   timeline reads, but keeps the whole entry instead of a 220-character summary, so the
   app can show the rule, when it applies and who proved it. A pillar is never stated
   above its evidence: that is CONFIDENCE-FRAMEWORK.md's law, not a preference. ── */
function pillarEntries(fileName, via) {
  const file = path.join(HOME, 'bb-consultancy', fileName);
  let raw; try { raw = fs.readFileSync(file, 'utf8'); } catch (e) { return []; }
  const mtime = fs.statSync(file).mtime.toISOString().slice(0, 10);
  /* plain indexOf, never a built regex: the first cut escaped the asterisks wrongly and
     every pillar vanished in silence. A count is what caught it, which is why the
     generator prints one for every list it builds. */
  const line = (block, label) => {
    const tag = '**' + label + ':**';
    const k = block.indexOf(tag);
    if (k < 0) return '';
    const start = k + tag.length;
    const end = block.indexOf('\n', start);
    return block.slice(start, end < 0 ? block.length : end).trim();
  };
  const rows = [];   /* NOT "out": that is the global the whole build writes to, and shadowing it here left the client roster empty and every evidence name unrecognised */
  for (const block of raw.split(/\n(?=#{2,3} )/)) {
    const h = block.match(/^#{2,3} (P-[\w-]+)\s*\|\s*([^\n]+)/);
    if (!h) continue;
    if (/RETIRED/.test(block.slice(0, 220))) continue;
    const pattern = line(block, 'Pattern');
    if (!pattern) continue;
    const tierRaw = line(block, 'Tier');
    const tier = (tierRaw.match(/HYPOTHESIS|PATTERN|PRINCIPLE/) || [])[0] || '';
    if (!tier) continue;                      // ungraded is not a pillar, it is a note
    const nMatch = tierRaw.match(/n\s*=\s*(\d+)/);
    /* THE EVIDENCE LIST IS NOT FREE TEXT. A Tier line may carry prose after the
       count ("n=4: Mold 1 gaps derived, ... not clients. It is a build principle"),
       and splitting it on commas turned sentence fragments into client names: the
       page then said 48 clients when BB has 33. Every name is checked against the
       real roster and anything unrecognised is dropped, so a count on this page can
       only ever be clients the brain actually knows. */
    const key = x => String(x).toLowerCase().replace(/[^a-z0-9]/g, '');
    const roster = new Map((out.clients || []).map(c => [key(c.name), c.name]));
    const namesPart = tierRaw.slice(tierRaw.indexOf(':') + 1);
    const clients = tierRaw.indexOf(':') > -1
      ? [...new Set(namesPart.replace(/\)\s*$/, '').split(',')
          .map(x => roster.get(key(x)))
          .filter(Boolean))]
      : [];
    rows.push({
      id: h[1], name: h[2].trim().replace(/\.$/, ''), pattern,
      tier, n: nMatch ? +nMatch[1] : (clients.length || 1), clients,
      applyWhen: line(block, 'Apply when'),
      limits: line(block, 'Contradicts / limits') || line(block, 'Contradicts'),
      date: line(block, 'Last strengthened').slice(0, 10) || mtime,
      via,
    });
  }
  return rows;
...
{
  const RANK = { PRINCIPLE: 0, PATTERN: 1, HYPOTHESIS: 2 };
  const all = pillarEntries('PATTERNS.md', 'file').concat(pillarEntries('synthesis/PATTERNS-cloud.md', 'cloud'));
  const seen = new Set();
  ...
  console.log('pillars:', out.pillars.length, JSON.stringify(byTier),
    '· proven on 4+ clients:', out.pillars.filter(p => p.n >= 4).length,
