...
the Brain, a feed that was offline or a downstream job that had not run, was given the same power as
a check on the Brain itself being wrong.
...
**Proved, not assumed.** Bridge aged to 40 hours: 1 warning, exit 0, publishes. Mac forced offline:
3 warnings, exit 0, publishes. A client's chat lines injected into the published file: 2 red, exit 1,
blocked. The teeth are still there.

**The rule for any gate from here.** Before a check can stop a release, ask what it is protecting
